...
%When the lags be considered,the strength from variable beyond to the behind can be used.   
%  log为正时，看的是Y对x的影响时延，因此定位输出的第二行
...
% function [C,r]=convergence(x,y,lag,position,input_lenth,tau,dim)

% if lag>=0
%     x=x(position:position+input_lenth);
%     y=y(position-lag-input_lenth:position-lag);
if lag>=0
    x=x(lag+1:end);
    y=y(1:end-lag);
else
...
% SHIYAN=find(Xdiagstd>=nanmean(Xdiagstd),10)
% SHIYAN2=nanmean(Xdiagstd)
...
% var_s=var(xflat)-var(yflat);

% symbolic
% var_all=zeros(1,length(xflat));
% for leng=length(xflat):-10:100
%     xflat=get_symbolic_ts(xflat,leng);
%     yflat=get_symbolic_ts(yflat,leng);
%     var_s=var(xflat)-var(yflat);
%     var_all(leng)=var_s;
% end
% xflat=get_symbolic_ts(xflat,100);
% yflat=get_symbolic_ts(yflat,100);
...
% iter=floor(sm/20);
% for cut=0:19
%     kankanx=dxs(cut*iter+1:(cut+1)*iter);
%     kankany=dys(cut*iter+1:(cut+1)*iter);
%     var_dxs=std(dxs(cut*iter+1:(cut+1)*iter));
%     var_dys=std(dxs(cut*iter+1:(cut+1)*iter));
%     var_dxs_all(cut+1)=var_dxs;
%     var_dys_all(cut+1)=var_dys;
% 
% end
% 
% 
% [dxs,idxs]=sort(xflat(xflat>0.4475));
% [dys,idys]=sort(yflat(yflat>0.8617));   
% 
% sm=length(xflat)
...
se_ycx=(r_ycx-rank).^2;
% figure()
% scatter(1:length(se_xcy),se_xcy)
...
count=[1:1:tm].';
shishi=-1*diff_se_xcy(1:tm)./se_null(1:tm)
% cumsum(-1*diff_se_xcy(1:tm)./se_null(1:tm))
